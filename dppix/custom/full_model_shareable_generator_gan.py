...
class FullModelShareableGeneratorGAN(ShareableGenerator):
    ...
    def learnable_to_shareable_gan(self, gen_learnable: ModelLearnable, disc_learnable: ModelLearnable,
                                   fl_ctx: FLContext) -> Shareable:
        gen_dxo = model_learnable_to_dxo(gen_learnable)
        disc_dxo = model_learnable_to_dxo(disc_learnable)

        return DXO(data_kind=DataKind.COLLECTION, data={'gen': gen_dxo, 'disc': disc_dxo}).to_shareable()

    def shareable_to_learnable_gan(self, shareable: Shareable, fl_ctx: FLContext) -> Dict[str, ModelLearnable]:
        if not isinstance(shareable, Shareable):
            raise TypeError("shareable must be Shareable, but got {}.".format(type(shareable)))
        ...
        gen_weights = base_model['gen'][ModelLearnableKey.WEIGHTS]
        disc_weights = base_model['disc'][ModelLearnableKey.WEIGHTS]

        dxo = from_shareable(shareable)
        gen_dxo = dxo.data['gen']
        disc_dxo = dxo.data['disc']

        if gen_dxo.data_kind == DataKind.WEIGHT_DIFF:
            if gen_dxo.data is not None:
                model_diff = gen_dxo.data
                for v_name, v_value in model_diff.items():
                    gen_weights[v_name] = gen_weights[v_name] + v_value
        elif gen_dxo.data_kind == DataKind.WEIGHTS:
            gen_weights = gen_dxo.data
            if not gen_weights:
                self.log_info(fl_ctx, "No gen model weights found. Model will not be updated.")
            else:
                base_model['gen'][ModelLearnableKey.WEIGHTS] = gen_weights
        else:
            raise ValueError(
                "data_kind should be either DataKind.WEIGHTS or DataKind.WEIGHT_DIFF, but got {} for gen".format(
                    gen_dxo.data_kind)
            )

        if disc_dxo.data_kind == DataKind.WEIGHT_DIFF:
            if disc_dxo.data is not None:
                model_diff = disc_dxo.data
                for v_name, v_value in model_diff.items():
                    disc_weights[v_name] = disc_weights[v_name] + v_value
        elif disc_dxo.data_kind == DataKind.WEIGHTS:
            disc_weights = disc_dxo.data
            if not disc_weights:
                self.log_info(fl_ctx, "No disc model weights found. Model will not be updated.")
            else:
                base_model['disc'][ModelLearnableKey.WEIGHTS] = disc_weights
        else:
            raise ValueError(
                "data_kind should be either DataKind.WEIGHTS or DataKind.WEIGHT_DIFF, but got {} for disc".format(
                    disc_dxo.data_kind)
            )

        base_model['gen'][ModelLearnableKey.META] = gen_dxo.get_meta_props()
        base_model['disc'][ModelLearnableKey.META] = disc_dxo.get_meta_props()
        return base_model
