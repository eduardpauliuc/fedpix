...
workspace='/Users/eduardpauliuc/PycharmProjects/federated/workspacehe'
...
if [ ! -f "${workspace}/project.yml" ]; then
     echo "3" | nvflare provision
     cp "project.yml" ${workspace}/.
fi
...
for s in "site-1" "site-2" "site-3" $server_name ; do
...
prod_dir = "${prod_dir}"
wait_for_system_start(num_clients = 3, prod_dir = prod_dir, username = username, secure_mode = True)
END
