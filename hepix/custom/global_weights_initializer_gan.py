...
class GlobalWeightsInitializerGAN(ResponseProcessor):
    def __init__(
            self,
    ...
    def process_client_response(self, client: Client, task_name: str, response: Shareable, fl_ctx: FLContext) -> bool:
        ...
        gen_weights = dxo.data['gen']
        disc_weights = dxo.data['disc']
        if not gen_weights:
            self.log_error(fl_ctx, f"No gen model weights found from client {client.name}")
            return False
        if not disc_weights:
            self.log_error(fl_ctx, f"No disc model weights found from client {client.name}")
            return False
        ...
        if not self.final_weights and (
            ...
            self.final_weights = {'gen': gen_weights, 'disc': disc_weights}
    ...
    def final_process(self, fl_ctx: FLContext) -> bool:
        ...
        prop = {'gen': make_model_learnable(self.final_weights['gen'], {}),
                'disc': make_model_learnable(self.final_weights['disc'], {})}

        fl_ctx.set_prop(self.weights_prop_name, prop, private=True, sticky=True)
        return True
