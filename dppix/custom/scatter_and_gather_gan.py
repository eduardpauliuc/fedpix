...
from file_model_persistor import PTFileModelPersistorGAN
...
class ScatterAndGatherGAN(Controller):
    def __init__(
            self,
            min_clients: int = 1,
            num_rounds: int = 5,
        ...
        if not isinstance(persistor_id, str):
            raise TypeError("persistor_disc_id must be a string but got {}".format(type(persistor_id)))
    ...
    def start_controller(self, fl_ctx: FLContext) -> None:
        ...
        if not isinstance(self.persistor, PTFileModelPersistorGAN):
            self.system_panic(
                f"Gen Model Persistor {self.persistor_id} must be a PTFileModelPersistor type object, "
        ...
        self._gen_weights, self._disc_weights = self.persistor.load_models(fl_ctx)

        self._global_weights = {'gen': self._gen_weights, 'disc': self._disc_weights}

        if not isinstance(self._gen_weights, ModelLearnable) \
                or not isinstance(self._disc_weights, ModelLearnable):
            self.system_panic(
                reason=f"Expected global weights to be of type `ModelLearnable`"
                       f" but received {type(self._gen_weights)} gen and {type(self._disc_weights)} disc",
                fl_ctx=fl_ctx,
            )
            return

        if self._gen_weights.is_empty() or self._disc_weights.is_empty():
            if not self.allow_empty_global_weights:
                ...
                self._gen_weights = self._global_weights['gen']
                self._disc_weights = self._global_weights['disc']
    ...
    def control_flow(self, abort_signal: Signal, fl_ctx: FLContext) -> None:
        try:
            ...
            while self._current_round < self._start_round + self._num_rounds:
                ...
                # Create train_task
                data_shareable: Shareable = self.shareable_gen.learnable_to_shareable_gan(self._gen_weights,
                                                                                          self._disc_weights, fl_ctx)
                ...
                self._global_weights = self.shareable_gen.shareable_to_learnable_gan(aggr_result, fl_ctx)
                fl_ctx.set_prop(AppConstants.GLOBAL_MODEL, self._global_weights, private=True, sticky=True)
                fl_ctx.sync_sticky()
                ...
                if (
                    ...
                    self.persistor.save_models(self._gen_weights, self._disc_weights, fl_ctx)
    ...
    def _process_train_result(self, client_task: ClientTask, fl_ctx: FLContext) -> None:
        result = client_task.result  # Collection Shareable DXO
