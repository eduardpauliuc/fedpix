...
class HEModelEncryptor(DXOFilter):
    def __init__(
        ...
        if isinstance(encrypt_layers, list):
            ...
            self.pattern = re.compile(encrypt_layers) if encrypt_layers else None
        else:
            self.encrypt_layers = [True]  # needs to be list for logic in encryption()
            self.logger.info("Encrypting all layers")

        self.logger.info("REGISTERING DECOMPOSERS")
    ...
    def encryption(self, params, fl_ctx: FLContext):
        ...
        # self.log_info(fl_ctx, f"XYZ params {params}")
        self.log_info(fl_ctx, f"XYZ regex {self.encrypt_layers}")
        # parse regex encrypt layers
        if isinstance(self.pattern, re.Pattern):

            self.log_info(fl_ctx, f"XYZ it is a reged")
            re_pattern = self.pattern
            # self.encrypt_layers = []
            for var_name in params:
                if re_pattern.search(var_name) and var_name not in self.encrypt_layers:
