...
class PTFileModelPersistorGAN(FLComponent):
    ...
    def load_models(self, fl_ctx: FLContext) -> Tuple[ModelLearnable, ModelLearnable]:
        ...
        if self.filter_id:
            ...
            gen_ml = _filter.process_post_load(learnable=gen_ml, fl_ctx=fl_ctx)
            disc_ml = _filter.process_post_load(learnable=disc_ml, fl_ctx=fl_ctx)
    ...
    def save_models(self, gen_ml: ModelLearnable, disc_ml: ModelLearnable, fl_ctx: FLContext):
        if self.filter_id:
            ...
            if not isinstance(_filter, PersistorFilter):
                raise ValueError(f"Expected filter to be of type `PersistorFilter` but got {type(filter)}")
            gen_ml = _filter.process_pre_save(learnable=gen_ml, fl_ctx=fl_ctx)
            disc_ml = _filter.process_pre_save(learnable=disc_ml, fl_ctx=fl_ctx)
        ...
        if self.filter_id:
            _filter.process_post_save(learnable=gen_ml, fl_ctx=fl_ctx)
            _filter.process_post_save(learnable=disc_ml, fl_ctx=fl_ctx)
    ...
    def get_model(self, model_file: str, fl_ctx: FLContext, model: str) -> ModelLearnable:
        try:
            ...
            model_ml = persistence_manager.to_model_learnable(self.exclude_vars)

            if self.filter_id:
                _filter = fl_ctx.get_engine().get_component(self.filter_id)
                if not isinstance(_filter, PersistorFilter):
                    raise ValueError(f"Expected filter to be of type `PersistorFilter` but got {type(filter)}")
                model_ml = _filter.process_post_get(learnable=model_ml, fl_ctx=fl_ctx)
